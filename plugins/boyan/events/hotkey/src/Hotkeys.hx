...
@:keepSub @:expose class Hotkeys
{
	...
	{
		pathToData = Node.path.join(HIDE.getPluginPath(Main.name), "config.json");
		parseData();
	...
	private static function addHotkey(menuItem:String, ?hotkeyText:String = "")
	{
		...
		else
		{
			...
			var data:String = HIDE.stringifyAndFormat(data);
			js.Node.fs.writeFileSync(Node.path.join(HIDE.getPluginPath(Main.name), "config.json"), data, js.Node.NodeC.UTF8);
		}
	...
	private static function parseData():Void
	{
		...
		data = Node.json.parse(Node.fs.readFileSync(pathToData, options));
	}
